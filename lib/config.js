/**
 * etl/config
 */
"use strict";

const logger = require('./logger');
const fs = require('fs');
const storage = require("@dictadata/storage-junctions");

var defaults = {
  "_log": {
    logPath: "./log",
    logPrefix: "etl",
    logLevel: "info"
  }
};

/**
 *
 */
exports.create = function (configFilename) {

  try {
    let sampleConfig = {
      "tract-name": {
        "origin": {
          "smt": "csv|./|foofile.csv|*",
          "options": {},
          "encoding": {}
        },
        "transforms": {
          "filter": {},
          "select": {}
        },
        "terminal": {
          "smt": "json|./|foofile.json|*",
          "options": {},
          "encoding": {},
          "create": true
        }
      },
      "_plugins": {
        "filesystems": {
          "name": ["model"]
        },
        "junctions": {
          "name": ["model"]
        }
      },
      "_log": {
        "logPath": "./log",
        "logPrefix": "etl",
        "logLevel": "info"
      }
    };

    logger.info("writing sample config to ./etl_config.json");
    fs.writeFileSync("./etl_config.json", JSON.stringify(sampleConfig, null, " "), { encodign: 'utf-8', flag: 'wx' });
  }
  catch (err) {
    logger.warn(err.message);
  }

};

/**
 *
 */
exports.load = function (configFilename) {
  let config = {};

  try {
    // read the app config file
    let appConfig = JSON.parse(fs.readFileSync(configFilename, 'utf-8'));
    config = Object.assign(this, defaults, appConfig);

    logger.configETLLogger(config._log);

    // check tract properties
    for (let [name, tract] of Object.entries(config)) {
      if (name === "_log") continue;
      if (name === "_plugins")
        loadPlugins(config["_plugins"]);

      if (typeof tract === "function") continue;
      // check origin properties
      if (typeof tract.origin !== "object") {
        throw new Error("invalid tract config: " + name);
      }
    }

  }
  catch (err) {
    logger.error(err.message);
  }

  return config;
};

async function loadPlugins(plugins) {

  // register filesystem plugins
  for (let [name, models] of Object.entries(plugins["filesystems"])) {
    let sfs = require(name);
    for (let model of models)
      storage.FileSystems.use(model, sfs);
  }

  // register junction plugins
  for (let [name, models] of Object.entries(plugins["junctions"])) {
    let stj = require(name);
    for (let model of models)
      storage.use(model, stj);
  }

}
