...
module.exports = () => {
  ...
  try {
    ...
    if (command === "convert") {
      ...
      if (argv.length > 2) {
        let params = querystring.parse(argv[2]);

        // assume that config.source exists
        if (!config.source.options)
          config.source.options = {};
        if (!config.source.options.params)
          config.source.options.params = {};

        Object.assign(config.source.options.params, params);
      }
    }
